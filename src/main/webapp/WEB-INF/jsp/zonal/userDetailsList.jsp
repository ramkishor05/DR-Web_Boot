...
<form name="profileForm" method="POST" action="${path}/deptHead/userManagement">

  			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />

</form>
<form name="deleteUserForm" method="POST" action="${path}/deptHead/deleteUser">
	 ...
	 <div class="container-fluid">
	    ...
	    <div class="dashboard-content">
	        <div class="row">
	            <div class="col-md-12">
	             <!-- <div class="text-right marB10">
	            	<button id="download-btn" class="btn btn-default">Download <i class="far fa-file-pdf" style="font-size: 120%;margin-left:3px;"></i></button>
	            </div>    -->
	                <div class="bg-white">
	                    ...
	                    <div id="table-content" class="block-content padBlock">
	                        ...
	                        <table class="table table-border-bottom">
	                            <thead>
	                                <tr>
	                                    <th class="w5">No.</th>
	                                    <th class="w15 text-left">First Name</th>
	                                    <th class="w10 text-left">Last Name </th>
	                                    <th class="w20 text-left">Email</th>
	                                    <th class="w15 text-left">Contact Number</th>
		                             	...
		                             	<%--<c:forEach items="${user.loginInfoCollection}" var="loginInfo" >
				                             ...
				                             <c:if test="${fn:containsIgnoreCase(activeStatus,'Y')}"> --%>
				                                <tr>
				                                    ...
				                                    <td>${user.profileId.firstName}</td>
				                                    <td>${user.profileId.lastName}</td>
				                                    <td>${user.profileId.email}</td>
				                                    <td>${user.profileId.phone}</td>
				                                    <td>${user.profileId.zoneId.zoneName}</td>
				                                    <td>
				                                        <a class='icon edit' href='javascript:createForm(${user.profileId.profileId});'></a>
				                                        <%-- <a class="icon delete" href="javascript:createDeleteForm(${user.profileId});"></a> --%>
				                                        <a class="icon delete" href="javascript:showDeleteModal(${user.profileId.profileId});"></a>
				                                    </td>
				                                </tr>
			                               <%--  </c:if>
		                              	</c:forEach>  --%>
...
<!-- Success  Modal -->
<div class="modal fade" tabindex="-1" role="dialog" id="updateConfirmModal"
	aria-labelledby="updateConfirmModal">
